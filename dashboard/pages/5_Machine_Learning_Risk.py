...
if selected_region == 'Alibori':
    ...
    narrative_p1 = t("<b>Analyse de la machine :</b> Situation extrêmement calme. La majority des actions recensées sont catégorisées comme 'coopératives'.", "<b>Machine analysis:</b> Extremely calm situation. The majority of recorded actions are categorized as 'cooperative'.")
...
# --- 5. STRATEGIC DIFFUSION MAP (PyDeck 3D) ---
st.markdown(f"### 📍 {t('Carte Tactique de Diffusion du Risque (3D)', 'Tactical Risk Diffusion Map (3D)')}")
st.markdown(f"""
<div style="font-size: 1.05rem; color: #c9d1d9; margin-bottom: 10px;">
    {t("Voici la projection spatiale du modèle de renseignement. Les piliers 3D indiquent l'intensité du risque détecté par l'IA pour les 7 prochains jours.", "Here is the spatial projection of the intelligence model. 3D pillars indicate the risk intensity detected by the AI for the next 7 days.")}
</div>
""", unsafe_allow_html=True)

# Data for the map (Mocked based on the prediction table above)
map_data = pd.DataFrame([
    {"name": "Alibori", "lat": 11.2, "lon": 2.9, "risk": 88.4, "color": [255, 123, 114, 200]},
    {"name": "Atacora", "lat": 10.3, "lon": 1.4, "risk": 74.1, "color": [255, 123, 114, 200]},
    {"name": "Borgou", "lat": 9.35, "lon": 2.6, "risk": 46.2, "color": [210, 153, 34, 200]},
    {"name": "Collines", "lat": 8.0, "lon": 2.3, "risk": 22.8, "color": [63, 185, 80, 200]},
    {"name": "Littoral", "lat": 6.36, "lon": 2.4, "risk": 12.5, "color": [63, 185, 80, 200]},
    {"name": "Atlantique", "lat": 6.6, "lon": 2.2, "risk": 15.0, "color": [63, 185, 80, 200]},
    {"name": "Oueme", "lat": 6.5, "lon": 2.6, "risk": 18.0, "color": [63, 185, 80, 200]},
])
...
# View state
view_state = pdk.ViewState(latitude=9.3, longitude=2.3, zoom=6, pitch=45)

# Render the deck
st.markdown('<div style="border: 1px solid #30363d; border-radius: 8px; overflow: hidden;">', unsafe_allow_html=True)
r = pdk.Deck(
    layers=[layer], 
    initial_view_state=view_state, 
    map_provider="carto",
    map_style=pdk.map_styles.CARTO_DARK,
    tooltip={"text": "{name}: {risk}% " + t("de risque", "risk")}
)
st.pydeck_chart(r)
